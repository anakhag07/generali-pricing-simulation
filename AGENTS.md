...
- **`src/experiments/configs/`** (preset registry)
  ...
  - `real_data_glm_softmax_policy_base.py`: GLM pickle-based softmax-policy base config; state_dim=12; unconstrained `l-bfgs-b`; all 5 estimators; analytical first-order gradient via u_coef
  ...
  - `real_data_glm_linear_policy_trust_region_constr.py`: constrained GLM linear-policy config with `LinearPolicy` and a trust-constr mean-acceptance floor set to the observed CSV acceptance level; enables all 5 estimators for constrained comparison
