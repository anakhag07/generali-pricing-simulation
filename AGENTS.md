...
- **`src/experiments/manifest.py`**
  - JSON manifest orchestration layer over `run_sweep(...)`: expands readable axes into named variants, supports direct preset/config overrides, composes declarative `noise` and `bias` objective wrappers, and skips variants whose expected per-seed summaries already contain required estimators. Keep it experiment-agnostic; experiment-specific post-run metrics and plots still belong in dedicated scripts.
...
- `scripts/run_experiment_manifest.py` is the readable JSON manifest launcher around `experiments.manifest.run_experiment_manifest(...)`. Use it for multi-axis grids involving seeds, direct config/factory overrides, optimizer/data ladders, and objective wrapper toggles (`noise`, `bias`) when no custom metric reconstruction is needed.
...
| `test_manifest.py` | JSON experiment manifest expansion, objective wrapper composition, completion skipping, and JAX requirement detection |
