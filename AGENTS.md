...
- `scripts/run_sweep.py` provides the dense planted-logistic homoskedastic fill-in sweeps using `sweep_utils.run_sweep(...)`: one theta-offset grid under `outputs/homoskedastic-theta-offset-sweep/` and one noise-std grid under `outputs/homoskedastic-noise-sweep/`. It skips completed variant folders that already contain both `finite_difference` and `stein_difference` results, then runs only missing variants with `RUN_SEEDS=(7,)`, `vary=("optimizer",)`, fixed `noise_seed=101`, and `CorrectnessSpec(gradient_source="denoised_exact")`; after execution it regenerates combined theta-distance-to-first-order-truth CSV/PNG plots for both grids. The script is CPU-only and auto-submits through the ORCD Slurm launcher unless `--no-sbatch` is passed for intentional local/debug execution
