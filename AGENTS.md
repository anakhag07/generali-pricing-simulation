...
- `scripts/run_noise_offset_grid.py` runs the combined noise-level x theta-offset planted-logistic grid with the **optax-adam** minimizer (`COMMON_OVERRIDES` step_rule=`optax-adam`, lr 0.05, t_steps 2000). For each noise family (homoskedastic std `sigma in {0, 0.1, 0.5, 2}`; heteroskedastic growth `gamma in {0, 0.25, 1, 4}`) it varies the init offset `delta` in `theta0 = theta_FO_clean + delta * 1` over 9 offsets with `RUN_SEEDS=(7, 8, 9)`. Every noise level runs fresh under optax-adam into dedicated `*-optax` project dirs (the l-bfgs-b reused-sweep collection is disabled to avoid mixing optimizers); `BASE_THETA` and `_first_order_truth_summary()` come from the optax-adam clean first-order truth run at `results/planted_logistic_base/first_order_truth_optax/`. Writes `noise_offset_grid_finals.csv` plus per-estimator two-panel figures (final theta distance to first-order truth | clean-objective gap, curves = noise level) under `results/homoskedastic-noise-offset-grid-optax/` and `results/heteroskedastic-noise-offset-grid-optax/`. `--plots-only` regenerates outputs from saved summaries; `--families` selects the grid group; launch-aware with auto Slurm submit (CPU: planted-logistic is a NumPy objective with no GPU path)
- `scripts/plot_noise_offset_grid_extra.py` adds diagnostic views on top of the finished noise-offset grids from `noise_offset_grid_finals.csv` (no rerun): single-axis slices along the noise axis (x=noise, curves=offset) and offset axis (x=offset, curves=noise), plus annotated `noise x offset` heatmaps of theta-distance-to-truth and clean-objective gap (LogNorm, brighter = larger drift from truth) that showcase optimizer noise-exploitation. Writes `noise_offset_slice_by_noise_<estimator>.png`, `noise_offset_slice_by_offset_<estimator>.png`, and `noise_offset_heatmap_<estimator>.png` into each `*-optax` project dir; `--families` selects the grid group
