...
files already exist unless `--force` is passed. An optional `reporting` array
attaches named, completion-aware post-run recipes to this same manifest format.
Example:
...
### Manifest-driven real-data reports

Reusable real-data analyses belong in the optional `reporting` array of the
existing experiment manifest format. Reports run during collection, are
completion-aware through `provenance.json`, and write under
`results/<manifest-name>/reports/<report-name>/`. No separate real-data
manifest kind is required.

For example, add this block to a normal manifest:
...
The `models` line is the model-selection seam. Choose any subset of `glm`,
`xgb`, `monotone_spline_xgb`, and `exact_spline_xgb` without copying analysis
code. The shared implementation resolves artifacts, eligible rows, model
frames, profit conventions, spline construction, statistics, PDF output, and
input/output hashes. Cohort indices and action-grid values are recorded so a
report can be replayed exactly.

New reusable feature analyses should follow the same pattern: add a validated
recipe under `src/experiments/reporting/`, put model/data calculations under
`src/reporting/` or `src/data/`, and keep the manifest as the source of run
parameters. Drivers for concluded analyses are retained only for provenance in
`scratch/retired_real_data_analysis/`; they are not supported entry points.
