...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--force",
        action="store_true",
        help="Rerun variants even when all requested seed summaries already exist.",
    )
    add_launch_args(parser, default_launch=None, default_array=None)
    return parser.parse_args(argv)


def _apply_manifest_launch_defaults(args: argparse.Namespace, manifest: ExperimentManifest) -> None:
...
def main(argv: list[str] | None = None) -> None:
    ...
    run_launch_plan(_build_launch_plan(args, manifest), args=args, argv=original_argv)
