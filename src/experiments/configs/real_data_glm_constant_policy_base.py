"""GLM-backed base config using a constant policy on real insurance data.

Uses the same trained GLM artifacts and raw state batch as the constrained
constant-policy preset, but runs unconstrained L-BFGS-B optimization.
"""
...
RUNTIME = canonical_runtime_block(
    ...
    verbose=True,
    wandb_enabled=True,
    wandb_project="glm-constant-policy-unconstrained",
)
