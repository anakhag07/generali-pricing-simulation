"""Constrained GLM quartic linear policy with full sphered policy features.

This preset keeps black-box artifact preprocessing sealed while the policy sees
the 10 raw acceptance-state columns after policy-side standardization and full
whitening/sphering with no PCA truncation. The observed GLM acceptance level is
enforced directly with SciPy's trust-region constrained solver.
"""
...
from objective.policy import LinearPolicy, QuarticFeatureMap
...
_policy = LinearPolicy(feature_map=QuarticFeatureMap())
...
_X_FIXED = load_x_array("glm", row_indices=_ROW_INDICES)
_X_POLICY = _X_FIXED[:, : len(ACCEPTANCE_STATE_COLS)]
_policy_preprocessor = fit_policy_feature_preprocessor(
    _X_POLICY,
    standardize=True,
    sphere=True,
    pca_dim=None,
...
THETA0 = np.zeros(_policy.theta_dim(_policy_preprocessor.output_dim_), dtype=float)

RUNTIME = canonical_runtime_block(
    plot=True,
    verbose=True,
    wandb_enabled=True,
    wandb_project="glm-linear-quartic-policy-no-pca-trust-region-constrained",
)
...
CONFIG = build_experiment_config(
    ...
    objective=make_model_based_objective(
        ...
        premium_col=9,
        u_coef=_u_coef,
        policy_preprocessor=_policy_preprocessor,
