"""Constrained GLM MLP policy with full sphered policy features.

The policy sees all 10 acceptance-state columns after policy-side
standardization and full whitening/sphering with no PCA truncation, while the
acceptance/loss artifacts keep their sealed preprocessing. The observed GLM
acceptance level is enforced directly with SciPy's trust-region constrained
solver.
"""
...
THETA0 = mlp_init_theta(
    ...
    d_in=_policy_preprocessor.output_dim_,
    hidden=HIDDEN,
)
...
RUNTIME = canonical_runtime_block(
    ...
    verbose=True,
    wandb_enabled=True,
    wandb_project="glm-mlp-policy-no-pca-trust-region-constrained",
)
