"""Constrained GLM linear-feature softmax policy with full sphered features.

This preset keeps black-box artifact preprocessing sealed while the softmax
policy sees the 10 raw acceptance-state columns after policy-side
standardization and full whitening/sphering with no PCA truncation. The
observed GLM acceptance level is enforced directly with SciPy's trust-region
constrained solver.
"""
...
_policy = SoftmaxPolicy(feature_map=IdentityFeatureMap())
...
RUNTIME = canonical_runtime_block(
    ...
    verbose=True,
    wandb_enabled=True,
    wandb_project="glm-softmax-linear-policy-no-pca-trust-region-constrained",
)
