"""Finite-policy lower-confidence-bound validation and reporting."""
...
LCBLaunchMode = Literal["auto", "local", "slurm"]
LCBLaunchArray = Literal["none", "seed"]
...
@dataclass(frozen=True)
class FinitePolicyLCBLaunchSpec:
    """Launch settings for the finite-policy LCB manifest."""

    mode: LCBLaunchMode
    array: LCBLaunchArray
...
def parse_finite_policy_lcb_manifest(
    ...
    *,
    ...
    _require_type(payload, "true_value", "identity")
    _require_type(payload, "surrogate", "policy_scaled_gaussian")

    seeds = _required_mapping(payload, "seeds")
    launch_payload = _required_mapping(payload, "launch")
    ...
    policies = _number_sequence(payload.get("policies"), "policies")
    deltas = _number_sequence(payload.get("deltas"), "deltas")
...
def analytic_joint_coverage(delta: float, policy_count: int) -> float:
    ...
    if int(policy_count) <= 0:
        raise ValueError("policy_count must be positive.")
    return float((1.0 - float(delta) / int(policy_count)) ** int(policy_count))
            ...
            rows.append(
                {
                    ...
                    "std_surrogate_value": _sample_std(surrogates),
                    "mean_lcb": float(np.mean(lcbs)),
                    "std_lcb": _sample_std(lcbs),
                }
            )
    return rows
...
def _coverage_summary_rows(
    ...
    for delta in spec.deltas:
        ...
        low, high = _wilson_interval(covered, len(group))
        rows.append(
            {
                "delta": delta,
...
def _require_type(payload: Mapping[str, Any], key: str, expected: str) -> None:
