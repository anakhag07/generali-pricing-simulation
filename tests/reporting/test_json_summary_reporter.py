...
def test_json_reporter_defaults_to_summary_json(tmp_path, monkeypatch) -> None:
    monkeypatch.setattr(json_summary, "build_summary_payload", lambda ctx, result: {"ok": True})
...
def test_json_reporter_writes_named_summary_in_variant_dir(tmp_path, monkeypatch) -> None:
    monkeypatch.setattr(json_summary, "build_summary_payload", lambda ctx, result: {"seed": 7})
