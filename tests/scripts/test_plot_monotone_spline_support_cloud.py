...
def _write_inputs(tmp_path):
    ...
    support_u = np.linspace(0.0, 0.16, 161)
    raw_width = np.linspace(10.0, 0.0, support_u.size)
    diagnostics = tmp_path / "coverage_diagnostics.npz"
    np.savez_compressed(
        diagnostics,
        row_indices=row_indices,
        u=support_u,
        median_support=np.linspace(1.0, 10.0, support_u.size),
        illustrative_width=raw_width,
        display_width=script._smooth(raw_width),
    )
...
def test_load_cloud_data_reuses_exact_sample_and_support_width(tmp_path) -> None:
    ...
    assert len(cloud) == 161
    np.testing.assert_allclose(cloud["u"], np.linspace(0.0, 0.16, 161))
...
def test_load_cloud_data_rejects_sample_mismatch(tmp_path) -> None:
    ...
    with pytest.raises(ValueError, match="different customer samples"):
        script._load_cloud_data(curve_csv, curve_manifest, diagnostics)
...
def test_run_analysis_writes_pdf_csv_and_manifest(tmp_path) -> None:
    ...
    assert len(pd.read_csv(outputs[1])) == 161
