...
def test_planted_logistic_minimum_at_u_star() -> None:
    ...
    grad_at_star = objective.grad_u_scalar(x, u_star)
    assert abs(grad_at_star) < 1e-8

    value_star = objective.value_scalar(x, u_star)
    value_left = objective.value_scalar(x, u_star - 0.1)
    value_right = objective.value_scalar(x, u_star + 0.1)
